...
type Role = "Coordinador" | "Chofer" | "Auxiliar";
...
export function OperativePortal({ role, session, initialAction=null }: { role: Role; session: Session; initialAction?: Action }) {
  ...
  async function submit(event:FormEvent) {
    ...
    if (action === "fuel" || action === "expense") {
      ...
      result = await supabase.from("expenses").insert({ user_id:session.user.id, service_id:form.service_id||null, vehicle_id:service?.vehicle_id||null, category:action === "fuel" ? "Gasolina" : form.category, concept:form.concept, amount:Number(form.amount), status:"Pendiente" });
