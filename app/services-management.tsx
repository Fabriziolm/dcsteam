...
export function ServicesManagement() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [serviceResult, clientResult, vehicleResult, staffResult] = await Promise.all([
      supabase.from("services").select("id,service_date,created_at,merchandise,origin,destination,scheduled_start,status,clients(name),vehicles(name,plate)").gte("service_date",range.start).lte("service_date",range.end).order("service_date", { ascending: false }).order("created_at", { ascending: false }).limit(100),
