...
export function HoursManagement({role}:{role:"Chofer"|"Auxiliar"}) {
  const [rows, setRows] = useState<Entry[]>([]),
    ...
    [selectedLunches,setSelectedLunches]=useState<Record<string,number>>({}),
    [loading, setLoading] = useState(true),
    [error, setError] = useState("");
  useEffect(() => {
    if (!supabase) return;
  ...
  const selectedLunchCount=useMemo(()=>selectedWeeks.reduce((sum,id)=>sum+(selectedLunches[id]||0),0),[selectedWeeks,selectedLunches]);
  const selectedBasePay=selectedMinutes/60*weekly.rate,selectedLunchPay=selectedLunchCount*20,selectedTotalPay=selectedBasePay+selectedLunchPay;
  ...
  return (
    <main className="content">
      ...
      <section className="panel data-panel">
        ...
        {importedHours.length===0?<div className="empty-state">Aún no hay horas históricas sincronizadas.</div>:<><div className="report-summary">{importedMonths.map(([month,value])=><div key={month}><span>{new Date(`${month}-02T12:00:00`).toLocaleDateString("es-PE",{month:"long",year:"numeric"})}</span><strong>{Math.floor(value.minutes/60)}h {value.minutes%60}m</strong><small>{value.weeks} semanas</small></div>)}</div><details className="week-selector"><summary><span>Calcular horas, sueldo y almuerzos</span><strong>{selectedWeeks.length}/4 semanas · S/ {selectedTotalPay.toFixed(2)}</strong></summary><div className="week-options"><div className="week-selection-result pay-result"><div><span>Horas seleccionadas</span><strong>{Math.floor(selectedMinutes/60)}h {selectedMinutes%60}m</strong></div><div><span>Pago base · S/ {weekly.rate.toFixed(2)}/h</span><strong>S/ {selectedBasePay.toFixed(2)}</strong></div><div><span>{selectedLunchCount} almuerzos · S/ 20</span><strong>S/ {selectedLunchPay.toFixed(2)}</strong></div><div><span>Total calculado</span><strong>S/ {selectedTotalPay.toFixed(2)}</strong></div><button type="button" onClick={()=>{setSelectedWeeks([]);setSelectedLunches({})}}>Limpiar</button></div>{importedHours.map(row=>{const selected=selectedWeeks.includes(row.id);return <label className={selected?"selected":""} key={row.id}><input type="checkbox" checked={selected} disabled={!selected&&selectedWeeks.length>=4} onChange={()=>toggleWeek(row.id)}/><span>Semana del {new Date(`${row.week_start}T12:00:00`).toLocaleDateString("es-PE",{day:"2-digit",month:"short",year:"numeric"})}</span><strong>{Math.floor(row.worked_minutes/60)}h {row.worked_minutes%60}m</strong>{selected&&<span className="lunch-count">Almuerzos <input type="number" min="0" max="6" value={selectedLunches[row.id]||0} onClick={event=>event.stopPropagation()} onChange={event=>setSelectedLunches(current=>({...current,[row.id]:Math.min(6,Math.max(0,Number(event.target.value)||0))}))}/></span>}</label>})}</div></details><div className="table-scroll"><table className="data-table"><thead><tr><th>Semana</th><th>Horas</th><th>Fuente</th></tr></thead><tbody>{importedHours.map(row=><tr key={row.id}><td>{row.week_start}</td><td><strong>{Math.floor(row.worked_minutes/60)}h {row.worked_minutes%60}m</strong></td><td>{row.source_value||"Google Sheets"}</td></tr>)}</tbody></table></div></>}
      </section>
...
function LegacyAdminWorkHoursReport(){
  ...
  useEffect(()=>{if(!supabase)return;void Promise.all([
    supabase.from("imported_work_hours").select("id,user_id,worker_label,week_start,worked_minutes,source_value").gte("week_start",range.start).lte("week_start",range.end).order("week_start",{ascending:false}),
...
export function AdminWorkHoursReport(){
  const {year}=useReportingYear(),range=yearRange(year),[hours,setHours]=useState<ImportedWorkHour[]>([]),[people,setPeople]=useState<Array<{id:string;name:string}>>([]),[worker,setWorker]=useState(""),[role,setRole]=useState<"Chofer"|"Auxiliar">("Chofer"),[selected,setSelected]=useState<string[]>([]),[lunches,setLunches]=useState<Record<string,number>>({}),[error,setError]=useState("");
  useEffect(()=>{if(!supabase)return;void Promise.all([supabase.from("imported_work_hours").select("id,user_id,worker_label,week_start,worked_minutes,source_value").gte("week_start",range.start).lte("week_start",range.end).order("week_start",{ascending:false}),supabase.from("profiles").select("id,full_name,email").eq("active",true).order("full_name")]).then(([h,p])=>{if(h.error||p.error)setError(h.error?.message||p.error?.message||"No se pudo cargar el personal.");else{const profiles=(p.data||[]).map(item=>({id:item.id,name:item.full_name||item.email||"Trabajador"}));const imported=(h.data||[]) as ImportedWorkHour[],labels=[...new Set(imported.map(item=>item.worker_label).filter(Boolean))].map(label=>({id:`label:${label}`,name:label}));setPeople([...profiles,...labels]);const normalized=imported.map(item=>item.user_id?item:{...item,user_id:`label:${item.worker_label}`});const existing=new Set(normalized.map(item=>item.user_id));const placeholders=profiles.filter(person=>!existing.has(person.id)).map(person=>({id:`profile-${person.id}`,user_id:person.id,worker_label:person.name,week_start:`${year}-01-01`,worked_minutes:0,source_value:"Sin horas importadas"}));setHours([...normalized,...placeholders]);setWorker(current=>current||profiles[0]?.id||labels[0]?.id||"")}})},[range.start,range.end,year]);
