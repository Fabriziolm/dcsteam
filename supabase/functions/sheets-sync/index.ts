...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const pruned=await db.rpc("prune_google_sheet_rows",{service_keys:keys(body.services),invoice_keys:keys(body.invoices),cash_keys:keys(cashRows),cash_balance_keys:keys(balanceRows),work_hour_keys:keys(workHourRows)});if(pruned.error)throw pruned.error;
