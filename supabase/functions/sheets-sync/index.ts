...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    // La limpieza debe recibir las claves originales del payload, iguales a las
    // que se guardan en source_key; así no elimina los registros recién insertados.
    const pruned=await db.rpc("prune_google_sheet_rows",{service_keys:keys(body.services),invoice_keys:keys(body.invoices),cash_keys:keys(body.cash),cash_balance_keys:keys(cashBalances),work_hour_keys:keys(workHours)});if(pruned.error)throw pruned.error;
