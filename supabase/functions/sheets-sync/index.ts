...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const workHours=Array.isArray(body.workHours)?body.workHours:[];
    ...
    const people=new Map((profiles||[]).flatMap(profile=>[profile.full_name,profile.email].filter(Boolean).map(value=>[norm(value),profile.id])));for(const profile of profiles||[]){const name=norm(profile.full_name);if(name.includes("nicolas"))people.set("nico",profile.id);if(name.includes("luis ortiz"))people.set("lolo",profile.id)}
    ...
    let serviceCount=0,invoiceCount=0,cashCount=0,workHoursCount=0;
    ...
    const workHourRows=workHours.filter(row=>row.sourceKey&&row.weekStart&&row.worker&&Number.isInteger(Number(row.minutes))&&Number(row.minutes)>=0).map(row=>({source_system:"google_sheets",source_key:row.sourceKey,user_id:people.get(norm(row.worker))||null,worker_label:String(row.worker).trim(),week_start:row.weekStart,worked_minutes:Number(row.minutes),source_value:row.sourceValue||null,updated_at:new Date().toISOString()}));
    for(const batch of chunks(workHourRows)){const hours=await db.from("imported_work_hours").upsert(batch,{onConflict:"source_system,source_key"});if(hours.error)throw hours.error;workHoursCount+=batch.length}
    // Los objetos ya normalizados usan snake_case; pasar sus source_key evita que
    // la limpieza posterior elimine todos los movimientos recién sincronizados.
    const pruned=await db.rpc("prune_google_sheet_rows",{service_keys:keys(body.services),invoice_keys:keys(body.invoices),cash_keys:cashRows.map(row=>row.source_key),cash_balance_keys:balanceRows.map(row=>row.source_key),work_hour_keys:workHourRows.map(row=>row.source_key)});if(pruned.error)throw pruned.error;
    ...
    return json({ok:true,services:serviceCount,invoices:invoiceCount,cash:cashCount,workHours:workHoursCount,syncedAt:new Date().toISOString()});
