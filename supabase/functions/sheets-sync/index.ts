...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const personAliases=new Map(people);for(const profile of profiles||[]){const name=norm(profile.full_name);if(name.includes("nicolas"))personAliases.set("nico",profile.id);if(name.includes("luis ortiz"))personAliases.set("lolo",profile.id)}
    const workHourRows=workHours.filter(row=>row.sourceKey&&row.weekStart&&row.worker&&Number.isInteger(Number(row.minutes))&&Number(row.minutes)>=0).map(row=>({source_system:"google_sheets",source_key:row.sourceKey,user_id:personAliases.get(norm(row.worker))||null,worker_label:String(row.worker).trim(),week_start:row.weekStart,worked_minutes:Number(row.minutes),source_value:row.sourceValue||null,updated_at:new Date().toISOString()}));
